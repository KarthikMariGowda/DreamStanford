...
public class ForgotPasswordCheck extends BaseTest {
	...
	public void forgotPasswordValid() throws InterruptedException {
		...
		for (int i = 1; i <= rowCount; i++) {
			...
			Reporter.log("Forgot password test case with valid data is passed");
			Thread.sleep(4000);
	...
	public void forgotPasswordInValid() throws InterruptedException {
		...
		for (int i = 1; i <= rowCount; i++) {
			...
			AssertJUnit.assertTrue(FpWarnMSG.equals("You must had login once. Before resetting your password.")//message needs a ,-BUG
					|| FpWarnMSG.equals("Email not found."));
					//|| FpWarnMSG.equals("Temporary password has expired and must be reset by an administrator."));

			Reporter.log("Forgot password test case with Invalid data is passed");
			Thread.sleep(4000);
			JavascriptExecutor js = (JavascriptExecutor) driver;
			js.executeScript("window.history.back()");
...
// need to write test case for Attempt limit exceeded, please try after some time- chk with dev team on how many attempts user email can get reset passwor sent to ur mail id " message and the email will br triggered to emil 
}