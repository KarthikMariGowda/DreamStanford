...
public class JsonDownloadInvalidSkeyTest extends BaseTest

{
	...
	public void jsonDownloadInvalidSkey() throws InterruptedException {
		...
		for (int i = 1; i <= rowCnt; i++) {
			...
			symmetricKeyPage.clearSymmetricKey();
			String inValidSkey = baseTest.read_XL_Data(XL_DATA_PATH, "SymmtricKeyInvalid", i, 0);
			symmetricKeyPage.sendSymmetricKey(inValidSkey);
	...
	public void jsonDownloadIncorrectSkey() throws InterruptedException {
		...
		for (int i = 1; i <= rowCnt; i++) {
			...
			symmetricKeyPage.sendSymmetricKey(inCorrectSkey);
			symmetricKeyPage.sK_clickOk();
			Thread.sleep(3000);
