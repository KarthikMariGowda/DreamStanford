...
public class NavigationCheckUploadsLink extends BaseTest
{
	...
	public void jsonDownloadCheck()  throws InterruptedException 
	{
		...
		ParticipantPage participantPage= new ParticipantPage(driver);
			 ...
			 Thread.sleep(4000);
			 boolean personalTabStatus= participantPage.personalTabiSSelected();
			 AssertJUnit.assertEquals(personalTabStatus, "true");
			 Reporter.log("Navigation flow on 0 uploads link click testcase is passed",true);
			 }
			 else
			 {
				 participantListPage.clickUploads();
				 commons.participantScreenWait(driver);
				 Thread.sleep(4000);
				 boolean viewUploadsStatus= participantPage.viewUploadsTabiSSelected();
				 Reporter.log(""+viewUploadsStatus,true);
				
				 AssertJUnit.assertEquals(viewUploadsStatus, true);
				 Reporter.log("Navigation flow on "+uploadsCount+" link click testcase is passed",true);
